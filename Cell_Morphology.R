...
# Morphology of Cells via Form Factor and Eccentricity ==================
...
df %>%
  ...
  select(AreaShape_Eccentricity_Cells, surface, material_type, ImageNumber_Image) %>%
  ...
  mutate(mean_eccentricity = mean(AreaShape_Eccentricity_Cells)) %>%
  # drop individual measure for eccentricity
  select(!AreaShape_Eccentricity_Cells) %>%
  # remove repeats
  distinct() %>%
  # plot
  ...
  labs(
    title = 'Cellular Eccentricity by Surface Type'
  ) +
  theme_minimal() +
  ylim(0.66, 0.78) +
...
df %>%
  # select relevant columns
  select(AreaShape_Eccentricity_Cells, surface, Neighbors_NumberOfNeighbors_Adjacent_Cells, ImageNumber_Image) %>%
  ...
  mutate(mean_ecc = mean(AreaShape_Eccentricity_Cells)) %>%
...
df %>%
  # select relevant columns
  select(AreaShape_FormFactor_Cells, surface, material_type, ImageNumber_Image) %>%
  # filter only for wanted groups
  filter(surface %in% c('flat_medium_density', 'nN_2_700', 'nN_2_1000', 'nN_5_700', 'nN_5_1000')) %>%
  # set the surface type to factor and set the order for plotting
  mutate(surface = as.factor(surface)) %>%
  ...
  mutate(mean_ff = mean(AreaShape_FormFactor_Cells)) %>%
  # drop individual measure for eccentricity
  select(!AreaShape_FormFactor_Cells) %>%
  # remove repeats
  distinct() %>%
  # plot
  ...
  labs(
    title = 'Cellular Form Factor by Surface Type'
  ) +
  theme_minimal() +
  scale_fill_manual(values = c('grey', '#f8961e', '#f3722c', '#90be6d', '#43aa8b')) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
df %>%
  # select relevant columns
  select(AreaShape_FormFactor_Cells, surface, Neighbors_NumberOfNeighbors_Adjacent_Cells, ImageNumber_Image) %>%
  # put number of neighbors into clear bins for plotting
  mutate(neighbors = case_when(
    Neighbors_NumberOfNeighbors_Adjacent_Cells == 0 ~ '0',
    Neighbors_NumberOfNeighbors_Adjacent_Cells >= 1 & Neighbors_NumberOfNeighbors_Adjacent_Cells < 4 ~ '1-3',
