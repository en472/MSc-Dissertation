# GMM normalisation for Cyclin A staining
...
feature <- "Intensity_MeanIntensity_CorrectedCycA_Nuclei"
...
df <- df %>%
  filter(.data[[feature]] <= 0.5)  # exclude values above 6n, e.g. very bright contaminants or multiple merged nuclei
...
## Optional: visual inspection of the normalised DNA content to compare to the original data
...
df <- df %>%
  rename_with(~ gsub("peak1", "CycA_neg", .x))

# write csv
write.csv(df, 'object_data_DNA_and_CycA_Normalised.csv')
